# putting this code in the excel file
...
#after installing the openpyxl using pip we import the following code
import os
import openpyxl
...
def enter_data():
    ...
    if accepted=="Accepted":
        ...
        if firstname and lastname:
            ...
            filepath = "C:\\Users\\DELL\\Desktop\\damason\\tkinter\\data.xlsx"

            if not os.path.exists(filepath):  #so if an excel sheet doesnt exist in our file path this function will create one
                workbook = openpyxl.Workbook()
                sheet = workbook.active  # this means it shouid work with an excel sheet that is open
                heading = ["First name", "Last name", "Title", "Age", "comlete course"
                           , "Registration status"]
                sheet.append(heading)
                workbook.save(filepath)
            workbook = openpyxl.load_workbook(filepath) # this will now load the data we put in on the excel file
            sheet = workbook.active
            sheet.append([firstname, lastname, title, age, registration_status, numcourses])
            workbook.save(filepath) 
